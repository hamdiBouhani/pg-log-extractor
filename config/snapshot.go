...
func SnapshotData(session *types.Session, requestParams *types.SnapshotDataJSON) ([]map[string]interface{}, error) {
	...
	if requestParams.OrderBy == nil {
		...
		if requestParams.Limit == nil && requestParams.Offset == nil {
			query = fmt.Sprintf("SELECT * FROM %s", requestParams.Table)
		} else {
		...
		if requestParams.Limit == nil && requestParams.Offset == nil {
			query = fmt.Sprintf("SELECT * FROM %s ORDER BY %s %s ", requestParams.Table, requestParams.OrderBy.Column, requestParams.OrderBy.Order)
		} else {
